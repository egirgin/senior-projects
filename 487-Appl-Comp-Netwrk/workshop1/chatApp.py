...
def discoverNetwork():
    ...
    for ip in allIPs:
        discoverClient = subprocess.Popen(["nc", "-q", "0", ip, "12345"], stdin = discoverProcess.stdout, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
...
def sendMessage(targetName, message, packetType):
    ...
    messengerClient = subprocess.Popen(["nc", "-q", "0", targetIP, "12345"], stdin = messageClient.stdout)
    print("Sending...")
